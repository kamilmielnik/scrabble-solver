...
import { selectRemainingTilesCount, selectRemainingTilesGroups, useTranslate, useTypedSelector } from 'state';
...
const RemainingTiles: FunctionComponent<Props> = ({ className, isOpen, onClose }) => {
  ...
  const remainingTilesCount = useTypedSelector(selectRemainingTilesCount);
  ...
  return (
    <Sidebar
      className={className}
      isOpen={isOpen}
      title={`${translate('remaining-tiles')} (${remainingTilesCount})`}
      onClose={onClose}
    >
