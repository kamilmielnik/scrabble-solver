...
| `error`        | `level, operation, locale, ip, ua, message, stack, input` | failed request (`level` `error`, `operation` is the route, `input` the first 1 KB of the request body or query), failed build (`operation` `build`), or a cached dictionary that had to be rebuilt (`level` `warn`, `operation` `cache`) |
...
- Failed requests produce only an `error` event, so every other type describes a successful request.

`logEvent()` appends synchronously and is silent under `bun test`. `logError()` writes an `error` event and mirrors it to stderr - the only thing this package prints. Do **not** import it from browser code.

## Export to CSV (Metabase)

```shell
bun run --filter @scrabble-solver/logger export           # every month
bun run --filter @scrabble-solver/logger export 2026-08   # one month
```

writes one RFC 4180 file per type and month to `$HOME/.scrabble-solver/csv/`:

```
builds-2026-08.csv  definitions-2026-08.csv  downloads-2026-08.csv  errors-2026-08.csv
solves-2026-08.csv  verifications-2026-08.csv  visits-2026-08.csv
```

The header is `timestamp` followed by the type's fields; empty cells are absent fields. Upload a month's file to Metabase to create a model, then **Append** each following month's file to it. Metabase requires identical column names, order and types for appends, which is why `EVENT_FIELDS` in `src/events.ts` is append-only: a new field means a new Metabase model. Metabase accepts files up to 50 MB; a month of the busiest type is well under that.
