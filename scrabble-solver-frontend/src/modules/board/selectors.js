...
export const selectCells = createSelector(selectRows, (rows) => rows.reduce(
  (cells, row) => cells.concat(row),
  []
));
export const selectResultCandidateCells = createSelector(selectResultCandidate, (result) => result ? result.cells : []);
export const selectRowsWithCandidate = createSelector(
  [ selectRows, selectResultCandidateCells ],
  (rows, cells) => rows.map(
    (row, y) => row.map(
      (cell, x) => getCell(cells, x, y) || cell
    )
  )
);
export const selectBonus = createCachedSelector(
